...
const WishlistTab = ({ wishlist }: WishlistTabProps) => {
    ...
    const paginatedWishlist = wishlist.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm text-center">
            {wishlist.length > 0 ? (
                ...
                <Empty
                    cn="border-0 shadow-none"
                    title="თქვენი სურვილების სია ცარიელია"
                    subTitle="დაამატეთ პროდუქტები რომლებიც მოგწონთ"
                    icon={Heart}
                    iconCn="w-16 h-16 mx-auto text-gray-500 mb-2"
                    extraBtn="დაიწყე შოპინგი"
                    extraBtnCn="mt-4"
                    extraBtnIcon={ShoppingCart}
                    onClick={() => navigate('/')}
                />
            )}
            ...
            {totalPages > 1 && (
                <div className="p-4 flex justify-center">
                    <CPagination page={page} setPage={setPage} totalPages={totalPages} />
                </div>
            )}
        </div>
    );
